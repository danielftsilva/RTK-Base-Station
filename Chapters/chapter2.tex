...
However, just three satellites are not enough for an accurate reading, although in theory this process should be enough. The practical use of trilateration must account with a minimum of four satellites in direct line of sight of any point on the surface of the Earth (usually to help clocks synchronisation).% Logically, it is immediately (and rightfully) assumed that the more satellites there are, the higher the positional accuracy will be for the receiver. For that, to achieve a successful observation, three major segments are necessary: the space segment, the control segment and the user segment.
% meter imagem word:
\begin{figure}[htbp]
	\centering
...
There are several networks of satellites from different nations around the world, with GPS being the most widely known and navigation-associated GNSS of all -- it was developed by the United States Department of Defense for military purposes, but was made available for civilian years later. % citation needed
...
There is also Dillution of Prescision (DOP): %https://en.wikipedia.org/wiki/Dilution_of_precision_(navigation)
