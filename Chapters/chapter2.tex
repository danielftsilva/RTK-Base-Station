...
Not only intended just for photography, UAVs (e.g. a drone) nowadays are capable of aiding in the most diverse ways, such as in the following examples, which correspond to some of the possibilities offered by Beyond Vision's HEIFU\textsuperscript{\texttrademark}\footnote{Hexa Exterior Intelligent Flying Unit (\url{https://beyond-vision.pt/heifu}).} UAV:
...
\end{itemize}
For these missions, the use of a base station capable of fine-tuning the UAV's position in real time is a big advantage. Therefore, it has to be designed with the ability of helping the UAV succeed in such missions in mind, which means its list of capacities should comprise enhanced landing, precise field surveying, accurate mapping and also work processes review.
...
Additionally, there are also two extra systems that were developed to provide regional coverage:
