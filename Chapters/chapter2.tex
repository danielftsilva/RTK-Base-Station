...
Regarding precise navigation, the precision obtained by solely relying on GNSS is not good enough (it has meter-level accuracy~\cite{novatel_gnss}) -- possible to conclude by looking at Table~\ref{tab:GNSS_sys_errors}. Taking the example of an autonomous drone used in all the processes mentioned in section \textbf{??}
%~\ref{sec:}
...
The way to do so is through techniques relying on the augmentation of GNSS positioning capabilities~\cite{novatel_gnss,kaplan_2017}, which portray an ingenious way of improving the four performance parameters described in section~\ref{sec:II_gnss} (accuracy, availability, continuity and integrity).
...
Unlike the conventional method presented in section~\ref{sec:II_gnss_comm}, DGNSS is a carrier-based method, which means the measurement of the carrier wave's phase corresponds to a measurement of the distance (commonly known as range) between the satellite and the receiver, expressed in number of cycles of the carrier frequency~\cite{inside_GNSS}. Outputs higher accuracy results.
...
reference station(s) provides information that is useful to improve PNT performance (accuracy, integrity, continuity, and availability) for the end user~\cite{kaplan_2017}.

The reference station(s) data may be supplied in real-time to the end user using any one of a variety of data links, for example, radio links at frequencies ranging from low frequencies (LF) below 300 kHz to L-band (1,000-2,000 MHz) and beyond, the Internet, and importantly, the link may not be real time. For instance, it is possible to implement DGNSS methods using two GNSS receivers that each simply log data to a hard drive or other storage device (PPK, section~\ref{sec:II_gnssAug_ppk}).
