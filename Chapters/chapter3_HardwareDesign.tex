...
The circuit of Figure~\ref{fig:BQ29209_circuit} is valid for a $V_{DD}$ supply voltage that ranges from 4V to 10V. In this case, $V_{DD}$ is equal to the battery pack's maximum voltage of 8.40V, meeting the spec. 


7.3 Recommended Operating Conditions
%aqui ver os valores maximos para RIN CIN RCB e RVD e CVD e CCD
7.6. Recommended Cell Balancing Configurations
Typical values stated where. . .
dizer aqui que meti RCB de 4.7k$\Omega$, so that the maximum cell balancing current that would run through the cells would not exceed 
7.7 meter um grafico ? talvez o da Figure 2, para falar da corrente de descarga

8.3.3 The recommended cell connection sequence begins from the bottom of the stack, as follows:
1. GND
2. VC1
3. VC2

%ou posso mentir e dizer que fiz como esta no esquematico recomendado, omitindo sempre a corrente minima de carga da bateria;
% ou posso colocar o circuito de 9.3.1 e dizer que meteria esse circuito.

8.3.4 To avoid prematurely discharging the cells, it is recommended to turn off (pull high) the active-low Cell Balance
Enable Control pin at lower State of Charge (SoC) levels.




Final note: the overvoltage protection feature is not indicated for the project's backup battery pack, since due to each cell's charging cycle termination voltage limit being 4.20V, this level would not meet the only two fixed overvoltage protection thresholds rated for the BQ20920x product family (4.30V or 4.35V, $\pm 0.25$mV) without causing damage overtime.
% REFERÊNCIA needed!!!!!!!!!!!!!!!!!!!!

%s--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--S

\subsubsection{Human-Machine Interface (HMI)}\label{sec:3213_BACKPANEL}
...
This circuit's function relies on an output signal by a D-type flip-flop -- named SN74LVC2G74DCTR, manufactured by Texas Instruments~\cite{SN74LVC2G74DCTR} --, which in turn allows or blocks the incoming power from either the external power adapter or the backup battery pack (i.e. from the PWR\_LTC net) to the voltage converter (Section~\ref{sec:3215_AP64501}), which powers the rest of the system.
This flip-flop output signal is, in turn, controlled by a push-button located on the outside of the base station casing, and when pressed connects the GND net to the input pin of a single Schmitt-trigger inverter -- named 74AHC1G14, by Diodes Incorporated~\cite{74AHC1G14}.
As this inverter has its supply voltage pin (pin 5 of 74AHC1G14) connected to the output of a low dropout regulator (LDO) -- MCP1790, by Microchip~\cite{MCP1790} --, its output voltage will be, when driven low, that same output voltage.
With this, a positive voltage impulse is established on the CLK pin of the SN74LVC2G74DCTR, and since this flip-flop relies on a single positive-edge-triggered architecture, this action activates it, setting its Q output high
%(as perTable~\ref{tab:SN74LVC2G74DCTR}),
driving the gate of the Q5 NMOS, subsequently shunting Q4 PMOS's gate to GND, driving it as well. This allows power from the mentioned PWR\_LTC net to the 5V voltage regulator, i.e. to the PWR\_AP64501 net. When in this state, once the switch is pressed again, the flip-flop's Q output is set to low, successively shutting down both Q5 and Q4 MOSFETs, blocking the passage of power and thus shutting down the system.
...
~\cite{SN74LVC2G74DCTR}
% REFERÊNCIA needed!!!!!!!!!!!!!!!!!!!! ~SN74LVC2G74DCTR

%s--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--Ss--S

\subsubsection{Voltage Converter}\label{sec:3215_AP64501}

Most of the main ICs of the base station run on a 5VDC. The best way to achieve that voltage level in this project is through the use of another DC-to-DC buck converter. The AP64501 (by Diodes Incorporated) synchronous buck converter was the selected part to establish the necessary 5V supply. This device's operation follows a PWM architecture at a typical switching frequency of 570kHz, being able to supply 5V, 12V or 24V, as well as up to 5A of continuous output current. It also generates a low quiescent current, which is very important since this IC is powered either from the external power adapter or the battery pack; a higher quiescent current could result in a non-negligible and unnecessary drain of the battery pack.

therefore a regulator needed in order to convert 
...
\subsubsection{GNSS Receiver, RTK Radio Link and Wi-Fi Transceiver}\label{sec:3231_ZEDF9P_XBEE3}
...
\subsubsection{USB Hub}\label{sec:3232_LAN9514}
