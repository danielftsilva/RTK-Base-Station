...
The CM4 provides a specially dedicated pin to power bank 0 (and thus set the ``pull'' level to each GPIO pin), labelled GPIO\_VREF (pin 78 in Figure~\ref{fig:CM4_GPIO_circuit}), which can either be tied to the CM4's +1.8V or +3.3V supply output (pins 88 and 90, or pins 84 and 86, respectively). It should be noted that the load connected to these output pins must not surpass 600mA. As for the total load of the 28 GPIO pins, it should be kept below 50mA.

Besides the ability of standard GPIO operation, all the GPIO pins are able to carry out up to six alternative functions, which can be selected by modifying the address of a corresponding ``Function Select Register'', assigned to each pin. According to~\cite{BCM2711}, for the GPIO pins used in this project -- GPIO4, GPIO8 and GPIO9 (pins 54, 39 and 40, respectively, in Figure~\ref{fig:CM4_GPIO_circuit}) --, the Function Select Register is GPFSEL0. All the alternative functions of the GPIO pins used in the project are presented in Table~\ref{tab:GPIO_alternative_functions}.
...
\begin{itemize}
	\item Pin 95, PI\_LED\_nPWR -- This is an active-low output used to drive an power-on LED indicator.
...
\paragraph{Additional Test Points}	To verify and debug the CM4's operation, e.g. to check its correct enabled and running status, a few test points were added to the design. Observing once again Figure~\ref{fig:CM4_GPIO_circuit}, the test points added are:
\begin{itemize}
	\item Pin 99, Global\_EN -- 
	\item Pin 100, nEXTRST -- 
	\item Pin 92, RUN\_PG -- 
	\item Pin 89, WL\_nDisable -- 
	\item Pin 91, BT\_nDisable -- 
	\item Pin 31, GPIO12 -- 
	\item Pin 28, GPIO13 -- 
	\item Pin 49, GPIO18 --
	\item Pin 26, GPIO19 -- 
...
Even though not every interface available on the CM4 is used, pins that represent some of them. Keeping in mind that there is always room for improvement, these test points' utility doubles, in the sense that they provide valuable pin status information that can be used to account for future versions of the beRTK\textsuperscript{\textregistered}.% arranjar maneira de meter uma frase sobre o debug e sobre o test de tensoes em features para versões futuras.
% ESTES TEXTOS DEVEM ESTAR ANTES DO \begin{itemize}

The CM4 also provides ways for the user to verify and debus its operation, via certain   selecting the main CM4 features to be used, there must be a way of status verification of these must be addressed, 
