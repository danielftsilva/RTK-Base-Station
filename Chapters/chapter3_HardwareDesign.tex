...
The LTC4012 datasheet provides a table with the minimum typical inductor values to be use as a guideline, which is represented by Table~\ref{tab:typ_inductor_values}. The construction of this table took into account maintaining a maximum $\Delta I_L$ value of $0.6 \cdot I_{MAX}=612$mA with $f_{PWM}=550$kHz and $V_{BAT}=0.5 \cdot V_{CLP}=16.8$V (which corresponds to the point of maximum $\Delta I_L$), for C-grade parts (corresponding to Grade 4 -- ``Commercial grade'' --, according to the Renesas AEC-Q100 Standard~\cite{AEC-Q100}), the expected grade for this project. Therefore, considering $V_{CLP}$ to be 16.8V and that the $I_{MAX}$ value found on Table~\ref{tab:typ_inductor_values} that is closest to the $I_{MAX}$ current for the new beRTK\textsuperscript{\textregistered} base station ($1,020$mA) is 1A, the recommended minimum typical inductor value should be chosen so that it is $\geq 20 \mu$H. It should be noted that the values for $R_{SENSE}$, $R_{IN}$ and $R_{PROG}$ are very similar to the determined/selected ones (98m$\Omega$, 3.01k$\Omega$ and 26.9k$\Omega$, respectively).
...
The L1 inductor must be carefully chosen due to the amount of ripple current $\Delta I_L$ it creates. 
The ripple current is a measure that decreases upon the rise of the PWM operating frequency ($f_{PWM}$) and/or the inductor's own inductance, being calulated through expression (\ref{eq:delta_iL}):

\begin{equation}\label{eq:delta_iL}
    \Delta I_L = \frac{V_{BAT} \cdot \left(1-\frac{V_{BAT}}{V_{CLP}}\right)}{L1 \cdot f_{PWM}}\,\medskip
\end{equation}

Considering L1 as $20 \mu$H, $V_{BAT}$ as 8.4V, the nominal PWM operating frequency as $f_{PWM}=550$kHz and $V_{CLP}$ as 16.8V (as instructed by the LTC4012 datasheet's inductor selection section), 0.382mA result as the $\Delta I_L$ ripple current value.% parei aqui

through (\ref{eq:delta_iL}), it is also possible to conclude that selecting lower values for indcutance will cause an increase in the indcutor's ripple current. This will inevitably lead to higher output voltage ripple -- the unwanted noise. 
